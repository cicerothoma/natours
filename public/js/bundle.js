...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiY2ljZXJvdGhvbWEiLCJhIjoiY2tmMzZsbTEwMDA5MjJybGZwaWM2YnRkeiJ9.Ph2iTxWF2xwIQTIYOc0Yuw';
  ...
  var bounds = new mapboxgl.LngLatBounds(); // Loops through the location array and adds a marker for each location

  locations.forEach(function (loc) {
    // 1) Creates the marker html element
    var el = document.createElement('div'); // 2) Adds the class "marker" for CSS styling

    el.className = 'marker'; // 3) Adds the marker element we created and style to mapbox

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // 4) Adds pop-up to describe our marker

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // 5) Extends the bounds of map to include current location
...
var stripe = Stripe('pk_test_51HTHBeHQW8t21RlGPtXSPGXio8JV5Dw36ymSuVeD0MUnsL4FCluPi7FBqWHPfxSzqyDiwUimipicEDzstm1DRMLc00AFZgoJGI');
...
var bookTourButton = document.getElementById('book-tour'); // VALUES
// DELEGATION
